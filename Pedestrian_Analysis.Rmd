...
LAT1 = min(LatestDataMeanbyDay$Latitude) - 0.001 ; LAT2 = max(LatestDataMeanbyDay$Latitude) + 0.001
...
p <- autoplot(map.latlon) +
  geom_point(data = LatestDataMeanbyDay %>% ungroup(),
             aes(x=Longitude, y=Latitude, fill=Mean, size=Mean), shape=21) +
  facet_wrap(~Day)
